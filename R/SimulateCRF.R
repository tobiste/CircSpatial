CDF2dir <- function(CircProb, CumProbZ, N, CircScale){
  n <- length(CircScale)
  CircProb[1] <- 0
  CircProb[n] <- 1
...
CDF_WrC <- function(Rho, CircScale){
  stopifnot(!(Rho == 0 | Rho >= 1))
...
CDF_Card <- function(Rho, CircScale){
  stopifnot(!(Rho == 0 | Rho > 0.5))
  # if (Rho == 0 | Rho > 0.5) stop("Cardioid: 0 < Rho <= 0.5")
  (CircScale + pi + 2 * Rho * sin(CircScale)) / (2 * pi)
}

r_Tri <- function(Rho, CumProbZ, N){
  direction <- vector(mode = "numeric", length = N)
...
SimulateCRF <- function(N = 100,
  ...
  if (CircDistr == "U") {
    ...
    direction <- r_Tri(Rho, CumProbZ, N)
  } else {
    ...
    if (CircDistr == "vM") {
      CircProb <- CDF_vM(Rho, CircScale)
    } else if (CircDistr == "Card") {
      CircProb <- CDF_Card(Rho, CircScale)
    } else if (CircDistr == "WrC") {
      CircProb <- CDF_WrC(Rho, CircScale)
    }
    direction <- CDF2dir(CircProb, CumProbZ, N, CircScale)
  }
