#' Title Circular kriging
#'
#' @param krig.x numeric
#' @param krig.y numeric
#' @param resid.x numeric. no na's
#' @param resid.y numeric. no na's
#' @param resid.direction numeric
#' @param Model select model from covariance models in R package `RandomFields::CovarianceFct()`
#' @param Nugget nugget component of the variogram (this basically adds a nugget compontent to the model); if missing, nugget component is omitted
#' @param Range range parameter of the variogram model component; in case of anisotropy: major range
#' @param sill sill of the variogram model component, or model:
...
#' @return
#' @import fields
#' @importFrom RandomFields CovarianceFct RFcov
#'
#' @export
#'
#' @examples
KrigCRF <- function(krig.x, krig.y, resid.x, resid.y, resid.direction, Model = RMexp(), Nugget = 0, Range, sill,
  ...
  for (i in 1:Ncol)
  {
    ...
    # ))
    K <- cbind(K, sill + (1 - Nugget - sill) * RFcov(
    x = Distances[, i] / Range, model = Model,
    params = list(
      M = 0,
      var = 1,
